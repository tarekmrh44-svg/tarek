let healthTimer  = null;
  ...
  async function doHealthCheck() {
    ...
    // لا يتوقف ابداً — يعيد العد ويكمل
    if (restartCount >= 5 && restartCount % 5 === 0) {
      console.log(`[MQTT_HEALTH] Still retrying... attempt #${restartCount + 1}`);
    }
    ...
    console.log(`[MQTT_HEALTH] No activity for ${silentMin} min — restart #${restartCount}`);

    if (restartCount <= 3) {
      notifyOwner(`⚠️ إعادة اتصال تلقائية\nلا نشاط منذ ${silentMin} دقيقة — محاولة #${restartCount}`);
    }
  ...
  function startHealthCheck() {
    ...
    const cfg = getConfig();
    if (!cfg.enable) return;
    console.log(`[MQTT_HEALTH] Started — unlimited retries, checks every 2–5 min`);
    scheduleNext();
  }

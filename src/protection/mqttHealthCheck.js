...
async function doHealthCheck() {
  ...
  if (restartCount >= cfg.maxRestarts) {
    // لا نوقف — نعيد العداد ونكمل بأطول backoff
    console.log(`[MQTT_HEALTH] Reached ${cfg.maxRestarts} restarts — resetting counter and continuing with max backoff`);
    notifyOwner(`⚠️ MQTT Health Check\n\nوصل البوت لـ ${cfg.maxRestarts} إعادة تشغيل\nيواصل المحاولة تلقائياً...`);
    restartCount = 0;

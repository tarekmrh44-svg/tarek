...
async function doRefresh() {
  ...
  try {
    const res = await axios.get(endpoint.url, {
      ...
      timeout: 15000,
      validateStatus: null,
      maxRedirects: 0, // لا نتبع الـ redirects — نكشف 302 مباشرة
    });

    // كشف انتهاء الجلسة: إما redirect لصفحة login أو HTML فيها علامات انتهاء الجلسة
    const location = res.headers?.location || "";
    const html     = typeof res.data === "string" ? res.data : "";
    const expired  =
      (res.status === 301 || res.status === 302) && location.includes("login") ||
    ...
    if (expired) {
      ...
      // حفظ AppState المحدَّث — يطيل عمر الكوكيز
      try {

...
// ─── Global Error Guards (يمنعون انهيار البوت نهائياً) ───────────────────────
process.on("uncaughtException", (err) => {
  try { log.error(`uncaughtException: ${err?.message || err}`); } catch (_) { console.error("uncaughtException:", err); }
});
...
function startWatchdog() {
  setInterval(async () => {
    ...
    const silent = Date.now() - (global._lastActivity || Date.now());
    if (silent > 20 * 60 * 1000) {
      log.warn(`⏱ Watchdog: لا نشاط منذ ${Math.round(silent / 60000)} دقيقة — إعادة الاتصال…`);
      global._lastActivity = Date.now();
      try { await global.reLoginBot(); } catch (_) {}
    }
  }, 5 * 60 * 1000); // يفحص كل 5 دقائق
}

// ─── تشغيل آمن مع إعادة محاولة تلقائية ──────────────────────────────────────
(async function safeMain() {
  let attempt = 0;
  while (true) {
    attempt++;
    try {
      if (attempt > 1) log.warn(`محاولة تشغيل رقم ${attempt}…`);
      await main();
      break; // main() عادةً لا تنتهي — لو انتهت نعيد المحاولة
    } catch (e) {
      log.error(`خطأ فادح في main: ${e?.message || e}`);
      const wait = Math.min(attempt * 10000, 60000);
      log.warn(`إعادة المحاولة بعد ${wait / 1000}s…`);

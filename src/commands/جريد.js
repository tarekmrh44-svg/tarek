"use strict";
/**
 * جريد — بث تلقائي دوري
 * أوامر: /جريد [رسالة] | /وقت [min] [max] | /جريدة ايقاف
 */

if (!global._broadcasts) global._broadcasts = new Map();

const sleep = ms => new Promise(r => setTimeout(r, ms));
const rand = (a, b) => a + Math.random() * (b - a);

async function scheduleBroadcast(api, threadID) {
  const job = global._broadcasts.get(threadID);
  if (!job || !job.running || !job.message) return;

  const delay = rand(job.minSec, job.maxSec) * 1000;

  const timer = setTimeout(async () => {
    const j = global._broadcasts.get(threadID);
    if (!j || !j.running) return;

    try {
      // تكامل مع أنظمة الحماية
      try { await require("../protection/connectionJitter").applyJitter("broadcast"); } catch (_) {}

      const typingMs = Math.min(j.message.length * 28, 3000) + rand(300, 800);
      try { if (api.sendTypingIndicator) api.sendTypingIndicator(threadID, () => {}); } catch (_) {}
      await sleep(typingMs);

      await new Promise((resolve, reject) => {
        api.sendMessage(j.message, threadID, (err) => err ? reject(err) : resolve());
      });
    } catch (_) {}

    if (global._broadcasts.get(threadID)?.running) scheduleBroadcast(api, threadID);
  }, delay);

  job.timer = timer;
}

function stopBroadcast(threadID) {
  const job = global._broadcasts.get(threadID);
  if (job?.timer) clearTimeout(job.timer);
  global._broadcasts.delete(threadID);
}

module.exports = {
  config: {
    name: "جريد",
    aliases: ["وقت", "جريدة"],
    description: "بث تلقائي دوري | /جريد [رسالة] — /وقت [min] [max] — /جريدة ايقاف",
    usage: "جريد [رسالة] | وقت [min] [max] | جريدة ايقاف",
    adminOnly: true,
    ownerOnly: false,
    category: "admin",
  },
  async run({ api, event, args, threadID, body, prefix, senderID }) {
    if (!global.isAdmin(senderID))
      return api.sendMessage("❌ هذا الأمر لأدمن البوت فقط.", threadID);

    const rawCmd = body.slice(prefix.length).trim().split(/\s+/)[0];

    // ── /جريدة ايقاف أو /جريد ايقاف
    if (rawCmd === "جريدة" || (rawCmd === "جريد" && args[0] === "ايقاف")) {
      if (!global._broadcasts.has(threadID))
        return api.sendMessage("⚠️ لا يوجد بث نشط في هذه المجموعة.", threadID);
      stopBroadcast(threadID);
      return api.sendMessage("🛑 توقّف البث التلقائي في هذه المجموعة.", threadID);
    }

    // ── /وقت [min] [max]
    if (rawCmd === "وقت") {
      const min = parseFloat(args[0]);
      const max = parseFloat(args[1]);
      if (isNaN(min) || isNaN(max) || min < 1 || max < min)
        return api.sendMessage("❌ الاستخدام: /وقت [min] [max] (بالثواني، مثال: /وقت 10 30)", threadID);

      let job = global._broadcasts.get(threadID) || {};
      if (job.timer) clearTimeout(job.timer);
      job.minSec = min; job.maxSec = max; job.running = !!job.message;
      global._broadcasts.set(threadID, job);

      if (!job.message)
        return api.sendMessage(`✅ تم ضبط التوقيت: ${min} — ${max} ثانية\nأرسل /جريد [رسالة] لبدء البث.`, threadID);

      job.running = true;
      scheduleBroadcast(api, threadID);
      return api.sendMessage(`✅ البث بدأ!\n📢 "${job.message}"\n⏱️ كل ${min}–${max} ثانية`, threadID);
    }

    // ── /جريد [رسالة]
    const message = args.join(" ").trim();
    if (!message)
      return api.sendMessage(
        "📡 الجريدة التلقائية:\n" +
        "• /جريد [رسالة] — حفظ رسالة البث\n" +
        "• /وقت [min] [max] — ضبط التوقيت وبدء البث\n" +
        "• /جريدة ايقاف — إيقاف البث\n\n" +
        (global._broadcasts.has(threadID)
          ? `📡 بث نشط: "${global._broadcasts.get(threadID).message}"`
          : "⚪ لا يوجد بث نشط"), threadID);

    let job = global._broadcasts.get(threadID) || {};
    if (job.timer) clearTimeout(job.timer);
    job.message = message; job.running = !!(job.minSec);
    global._broadcasts.set(threadID, job);

    if (!job.minSec)
      return api.sendMessage(`✅ تم حفظ الرسالة:\n"${message}"\n\nأرسل /وقت [min] [max] لبدء البث.`, threadID);

    scheduleBroadcast(api, threadID);
    return api.sendMessage(`✅ تم تحديث الرسالة وبدء البث!\n📢 "${message}"\n⏱️ كل ${job.minSec}–${job.maxSec} ثانية`, threadID);
  },
};

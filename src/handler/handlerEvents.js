...
// ─── AppState Auto-Save (debounced — كل 3 دقائق عند النشاط) ─────────────────
const fs   = require("fs-extra");
const path = require("path");
let _saveTimer = null;
function scheduleAppStateSave() {
  if (_saveTimer) return; // مرة واحدة فقط كل 3 دقائق
  _saveTimer = setTimeout(() => {
    ...
    try {
      ...
      const ACCOUNT = path.join(__dirname, "../../account.txt");
      global._selfWrite = true;
      fs.writeFileSync(ACCOUNT, JSON.stringify(dedup(fresh), null, 2), "utf8");
...
module.exports = async function handlerEvents(api, event, commands) {
  ...
  scheduleAppStateSave(); // حفظ AppState المحدَّث كل 3 دقائق عند النشاط
  ...
  // ══ GROUP EVENT ══════════════════════════════════════════════════════════════
    ...
    // ══ حماية اسم الغروب (صامتة — بدون أي رسالة) ═══════════════════
    if (logMessageType === "log:thread-name") {
      const protectedNames = global._protectedNames || new Map();
      if (protectedNames.has(threadID)) {
        const savedName = protectedNames.get(threadID);
        const newName   = logMessageData?.name || "";
        if (newName !== savedName) {
          // أعد الاسم فوراً بصمت
          try {
            await new Promise((res, rej) =>
              api.setTitle(savedName, threadID, e => e ? rej(e) : res())
            );
          } catch (_) {}
        }
      }
